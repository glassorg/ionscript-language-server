...
class DocumentSemanticTokensProvider implements vscode.DocumentSemanticTokensProvider {
	async provideDocumentSemanticTokens(document: vscode.TextDocument, token: vscode.CancellationToken): Promise<vscode.SemanticTokens> {
		...
		const { getSemanticHighlights } = require(root + "ionscript/lib/compiler/semanticHighlighting");
		let text = document.getText();

		const builder = new vscode.SemanticTokensBuilder();
		for (let { line, column, length, tokenType, modifiers} of getSemanticHighlights(text, document.fileName)) {
			builder.push(line, column, length, this._encodeTokenType(tokenType), this._encodeTokenModifiers(modifiers));
		}
